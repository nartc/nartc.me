...
Then go to `https://localhost:4321` and we'll see our `Experience` component is rendered 🎊
...
Additionally, we also need to updatee `astro.config.mjs` with the following change:
...
<BlogImage  src={astroBoxBig} alt="Astro home page a pink box on a large canvas" />
...
<BlogImage  src={astroBoxAnimateLights} alt="A 'real' rotating pink box" format="gif"/>
...
<BlogImage  src={astroBoxOrbitControls} alt="Controlling the camera with OrbitControls" format="gif"/>
